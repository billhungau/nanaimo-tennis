...
const questionShortLabels = ["Pause removal", "Evaluate alternatives", "Long-term planning"];
...
const codeMeta: Record<PositionCode, { label: string; className: string; barClass: string }> = {
  support: { label: "Support", className: "border-blue-200 bg-blue-50 text-blue-800", barClass: "bg-blue-600" },
  conditional: { label: "Conditional", className: "border-amber-200 bg-amber-50 text-amber-800", barClass: "bg-amber-600" },
  unclear: { label: "Unclear", className: "border-slate-300 bg-slate-100 text-slate-700", barClass: "bg-slate-500" },
  oppose: { label: "Does not support", className: "border-purple-200 bg-purple-50 text-purple-800", barClass: "bg-purple-600" },
};
...
function CandidatesPage() {
  ...
  function renderCodePill(question: "Q1" | "Q2" | "Q3", code: PositionCode) {
    ...
    return <span className={`inline-flex shrink-0 items-center gap-1 rounded-full border px-1.5 py-0.5 text-[9px] font-semibold leading-4 sm:gap-1.5 sm:px-2 sm:py-1 sm:text-[11px] md:px-1.5 md:py-0.5 md:text-[10px] ${meta.className}`}>
      <span className="size-2 shrink-0 rounded-full bg-current ring-2 ring-current/15 sm:size-2.5" aria-hidden="true" />
      {question} {meta.label}
    </span>;
  }
  ...
  function renderCandidate(candidate: CandidateRecord) {
    ...
    return <article key={candidateKey} className="border-b border-border bg-card last:border-0">
      <div
        className="grid cursor-pointer grid-cols-1 gap-1.5 px-4 py-2.5 transition-colors hover:bg-secondary/30 sm:gap-2 sm:px-5 sm:py-3.5 md:grid-cols-[minmax(0,2.2fr)_minmax(22rem,2.2fr)_7.5rem] md:items-center md:gap-3 md:py-4"
        onClick={toggleOpen}
      >
        <div className="min-w-0">
          <div className="flex flex-wrap items-center gap-1.5 sm:gap-2">
            <h3 className="font-serif text-base leading-snug sm:text-lg md:text-[19px]">{candidate.name}</h3>
            {currentRole && <span className="shrink-0 rounded-sm border border-border bg-secondary/70 px-1.5 py-0.5 text-[9px] font-medium text-muted-foreground sm:px-2 sm:text-[11px]">{currentRole}</span>}
          </div>
        </div>

        <div className="flex min-w-0 flex-nowrap items-center gap-1 overflow-hidden whitespace-nowrap md:justify-start">
          {received ? <>
            {codes.map((code, index) => code
              ? <span key={index} className="min-w-0 shrink-0">{renderCodePill(`Q${index + 1}` as "Q1" | "Q2" | "Q3", code)}</span>
              : <span key={index} className="inline-flex shrink-0 items-center rounded-full border border-border bg-secondary/50 px-1.5 py-0.5 text-[9px] font-semibold leading-4 text-muted-foreground sm:px-2 sm:py-1 sm:text-[11px] md:px-1.5 md:py-0.5 md:text-[10px]">Q{index + 1} Not coded</span>)}
          </> : <span className="rounded-sm border border-border bg-secondary/50 px-2 py-1 text-[10px] font-medium text-muted-foreground sm:text-xs">No response received</span>}
        </div>
      ...
      <div hidden={!isOpen} className="border-t border-border bg-secondary/25 px-4 py-5 sm:px-6 sm:py-8">
        {received && hasResponses ? <div className="mx-auto max-w-4xl">
          <div className="rounded-sm border border-border bg-background px-4 py-1 sm:px-7 sm:py-2">
            {candidateQuestions.map((question, index) => <section key={question} className="border-b border-border py-5 last:border-0 sm:py-7">
              ...
              <div className="mt-3 max-w-3xl sm:mt-4">{renderResponseText(responses[index])}</div>
            </section>)}
          </div>
  ...
  return <>
    ...
    <section className="border-b border-border bg-card py-6 sm:py-8">
      <div className="page-wrap">
        ...
        <div className="mt-5 grid gap-2.5 md:grid-cols-3 md:gap-3">
          {questionSummaries.map((summary, index) => <div key={summary} className="flex gap-3 border border-border bg-secondary/35 p-3.5 sm:p-4">
            ...
            <div>
              <p className="text-sm font-semibold leading-6 sm:hidden">{questionShortLabels[index]}</p>
              <p className="hidden text-sm leading-6 sm:block">{summary}</p>
            </div>
          </div>)}
        </div>
      ...
      <div className="page-wrap">
        ...
        <div className="mt-5 grid gap-3 md:grid-cols-3">
          {[
            ...
            const codeOrder: PositionCode[] = ["support", "conditional", "unclear", "oppose"];
            return <article key={question} className="border border-border bg-card p-4 sm:p-5">
              ...
              <div className="mt-3 flex h-2 overflow-hidden rounded-full bg-secondary" aria-label={`${question} response distribution`}>
                {codeOrder.map((code) => {
                  const percentage = receivedCount ? (counts[code] / receivedCount) * 100 : 0;
                  return percentage > 0 ? <span key={code} className={codeMeta[code].barClass} style={{ width: `${percentage}%` }} title={`${codeMeta[code].label}: ${Math.round(percentage)}%`} /> : null;
                })}
              </div>
              <div className="mt-4 space-y-2 text-sm">
                {codeOrder.map((code) => {
                  ...
                  return <div key={code} className="flex items-center justify-between gap-3">
                    <span className={`inline-flex items-center gap-2 ${textClass}`}><span className="size-2.5 rounded-full bg-current ring-2 ring-current/15" aria-hidden="true" />{codeMeta[code].label}</span>
    ...
    <section className="section-space">
      <div className="page-wrap">
        ...
        <div className="sticky top-[3.75rem] z-20 -mx-4 border-y border-border bg-background/95 px-4 py-3 shadow-sm backdrop-blur md:static md:mx-0 md:grid md:grid-cols-[1fr_auto] md:items-end md:gap-4 md:border-0 md:bg-transparent md:p-0 md:shadow-none">
          <div className="space-y-2.5 sm:space-y-3">
            ...
            <div className="flex flex-nowrap gap-1 overflow-x-auto pb-0.5 sm:flex-wrap sm:gap-2 sm:overflow-visible sm:pb-0" role="group" aria-label="Filter candidates by response status">
              <Button className="shrink-0" size="sm" variant={responseFilter === "all" ? "secondary" : "ghost"} onClick={() => setResponseFilter("all")}>All responses</Button>
              <Button className="shrink-0" size="sm" variant={responseFilter === "received" ? "secondary" : "ghost"} onClick={() => setResponseFilter("received")}>Responses received</Button>
              <Button className="shrink-0" size="sm" variant={responseFilter === "not_received" ? "secondary" : "ghost"} onClick={() => setResponseFilter("not_received")}>No response received</Button>
            </div>
          </div>

          <div className="relative mt-2.5 w-full md:mt-0 md:w-72">
